...
### 新增
- **多模态消息支持**：支持通过 Koishi 元素标签 (`<img>`, `<audio>`, `<video>`, `<file>`) 发送图片、音频、视频和文件。Agent 可通过系统提示词自动识别并生成对应格式。
- **基于目录的会话隔离**：新增 `directory` 配置项。修改此配置会自动创建新的隔离会话，确保不同工作区之间上下文互不干扰 (Title 包含目录 Hash)。
- **配置项**：新增 `directory` 配置，用于指定工作区目录。

### 修复
- **权限安全修复**：修复了 `config.authority` 默认值 (1) 意外覆盖敏感指令（如删除会话、设置模型）高权限要求的问题。现在敏感指令强制使用更高权限等级（2-4）。
- **指令文档**：完善了所有 `oc` 指令的帮助文档，包含详细描述、用法说明和使用示例 (`help oc`, `help oc.models` 等)。
- **创建会话权限**：将 `oc.session.new` 权限下调至 1，方便普通用户在隔离策略下手动重置会话。
